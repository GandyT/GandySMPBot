...
module.exports = {
    ...
    async execute(env) {
        ...
        */
        ...
        var mentioned = DiscordManager.findUser(args[1], message);
        if (!mentioned) return message.channel.send("**Please mention the user you wish to transfer ownership to**");
        var mentionedUser = DataManager.getUser(mentioned.id);
        if (mentioned.id == message.author.id) return message.channel.send("**You can not transfer ownership of the group to yourself**");
